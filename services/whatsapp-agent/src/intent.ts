...
export async function extractBookingIntent(
  ...
  const prompt = `Today is ${todayDate}. Extract booking intent from this customer message.
...
Return ONLY valid JSON with this exact shape:
{
  ...
  "date": "<YYYY-MM-DD resolved from today's year ${todayDate.slice(0, 4)}, or null>",
...
- Dates: "25.7" or "25/7" → "${todayDate.slice(0, 4)}-07-25"
