  ...
  return {
    async list(businessId: string) {
      const [svcRes, catRes] = await Promise.all([
        repo.findAll(businessId),
        repo.findCategories(businessId),
      ]);
    ...
    async add(businessId: string, body: Record<string, unknown>) {
      ...
      if (typeof color !== "string" || !/^#([A-Fa-f0-9]{6})$/.test(color))
        throw new AppError(400, "Invalid color format");
    ...
    async edit(serviceId: string, businessId: string, body: Record<string, unknown>) {
      if (
        body.color != null &&
        (typeof body.color !== "string" || !/^#([A-Fa-f0-9]{6})$/.test(body.color))
      )
        throw new AppError(400, "Invalid color format");
    ...
    async remove(serviceId: string, businessId: string) {
      const { error } = await repo.softDelete(serviceId, businessId);
      if (error) throw new AppError(400, error.message);
    },
  };
}
