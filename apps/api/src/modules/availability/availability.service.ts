    ...
    async get(businessId: string, serviceId: string, dateStr: string) {
      ...
      for (const wh of whR.data || []) {
        ...
        if (!wh.is_closed)
          whMap
            .get(wh.day_of_week)!
            .push({
              start: toUTCTime(wh.start_time, il.hours),
              end: toUTCTime(wh.end_time, il.hours),
            });
      }
      const workingHours: WorkingDay[] = Array.from({ length: 7 }, (_, day) => {
        ...
        return {
          ...
          isClosed:
            ranges.length === 0 ||
            (whR.data || []).some((wh) => wh.day_of_week === day && wh.is_closed),
        };
      });
      const breaks: BreakPeriod[] = (brR.data || []).map((b) => ({
        type: b.type as BreakPeriod["type"],
        dayOfWeek: b.day_of_week ?? undefined,
        specificDate: b.specific_date ?? undefined,
        start: toUTCTime(b.start_time, il.hours),
        end: toUTCTime(b.end_time, il.hours),
      }));
      const existing: ExistingAppointment[] = (aptR.data || []).map((a) => ({
      ...
      const svcCfg: ServiceConfig = {
        ...
        maxCapacity: availStaff ?? service.max_capacity,
      };
      const rules: BookingRulesConfig | undefined = rulesR.data
        ? {
            minAdvanceMinutes: rulesR.data.min_advance_minutes,
            maxFutureDays: rulesR.data.max_future_days,
            cancellationWindowMinutes: rulesR.data.cancellation_window_minutes,
            rescheduleWindowMinutes: rulesR.data.reschedule_window_minutes,
          }
        : undefined;
