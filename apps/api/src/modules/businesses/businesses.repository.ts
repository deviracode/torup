...
export function createBusinessRepo(client: SupabaseClient<Database>) {
  return {
    ...
    async findBySlugOrId(slugOrId: string) {
      ...
      return (
        isUUID
          ? client.from("businesses").select("*").eq("id", slugOrId)
          : client.from("businesses").select("*").eq("slug", slugOrId).eq("is_active", true)
      ).single();
    },
    async update(businessId: string, data: Record<string, unknown>) {
      return client.from("businesses").update(data).eq("id", businessId).select().single();
    },
  };
}
