...
export function createAdminService(repo: Repo) {
  return {
    async listBusinesses(filters?: { category?: string; status?: string; search?: string }) {
    ...
    async onboard(
      ...
      if (owner_email) {
        ...
        if (existing) {
          ownerId = existing.id;
          await supabase.auth.admin.updateUserById(existing.id, { password: tempPw });
        } else {
    ...
    async editBusiness(businessId: string, data: Record<string, unknown>) {
      const { data: biz, error } = await repo.updateBusiness(businessId, data);
