  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          <div className="col-span-full rounded-lg bg-background p-8 text-center text-muted-foreground border border-border">{tCommon("loading")}</div>
        ) : plans.map((plan) => (
          <div key={plan.id} className={`rounded-lg border bg-background p-6 ${plan.is_active ? "border-border" : "border-border opacity-60"}`}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-lg font-bold">{plan.name}</h3>
              <button onClick={() => openEdit(plan)} className="text-xs text-blue-600 hover:underline">{tCommon("edit")}</button>
            </div>
            <div className="space-y-2 text-sm">
              <p><span className="text-muted-foreground">{t("monthlyPrice")}:</span> {formatILS(plan.monthly_price)}</p>
              <p><span className="text-muted-foreground">{t("yearlyPrice")}:</span> {formatILS(plan.yearly_price)}</p>
              <p><span className="text-muted-foreground">{t("maxStaff")}:</span> {plan.max_staff}</p>
              <p><span className="text-muted-foreground">{t("maxAppointments")}:</span> {plan.max_appointments_monthly}</p>
            </div>
            <div className="mt-3">
              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${plan.is_active ? "bg-green-100 text-green-700" : "bg-muted text-muted-foreground"}`}>
