-- Fix 00039: breaks.specific_date is DATE, but the function compared it
-- against a TEXT value (to_char(...)) — Postgres has no date = text
-- operator, so every call failed with 42883 before ever reaching the
-- capacity check. Found by actually exercising the function against
-- staging rather than trusting the SQL by inspection alone.
